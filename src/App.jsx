...
import {
  ...
  trackVisit, trackItemClick, getAnalyticsSummary,
} from "./storage.js";
...
import {
  ...
  Sparkles, MessageCircle, FileUp, Moon, Sun, ScanLine,
} from "lucide-react";
                ...
                return (
                  <Card key={d.id} className="overflow-hidden group cursor-default">
                    ...
                    >
                      ...
                      {d.createdAt && Date.now() - d.createdAt < 14 * 24 * 60 * 60 * 1000 && (
                        <span className="absolute top-2.5 right-2.5 text-[10px] font-extrabold px-2 py-1 rounded-full bg-emerald-500 text-white shadow-sm z-10">
                          NEW
                        </span>
                      )}
                    </div>
                    <div className="p-3.5">
                      ...
                      {(() => {
                        const dl = daysLeft(d.dealEndDate);
                        if (dl === null) return null;
                        if (dl < 0) return null;
                        return (
                          <div className={`inline-flex items-center gap-1 text-[10px] font-bold px-2 py-0.5 rounded-full mt-1.5 ${dl <= 1 ? "bg-red-100 text-red-700" : "bg-amber-100 text-amber-700"}`}>
                            <Zap size={9} /> {dl === 0 ? "Ends today" : dl === 1 ? "1 day left" : `${dl} days left`}
                          </div>
                        );
                      })()}
                      ...
                      <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        onClick={() => trackItemClick(d.id, d.name)}
...
export default function InventoryApp() {
  ...
  const [darkMode, setDarkMode] = useState(() => {
    try {
      return localStorage.getItem("stockline-dark") === "1";
    } catch (e) {
      return false;
    }
  });
  React.useEffect(() => {
    try {
      localStorage.setItem("stockline-dark", darkMode ? "1" : "0");
    } catch (e) {}
  }, [darkMode]);
  ...
  const pendingDealsCount = items.filter((i) => i.dealStatus === "pending").length;
  const navBadges = { sales: role === "Admin" ? pendingSalesCount : 0, items: role === "Admin" ? pendingDealsCount : 0 };
  ...
  function saveItem(form) {
    if (form.id) {
      ...
      const newItem = { ...form, id: uid("I"), createdAt: Date.now() };
  ...
  // Parses a simple CSV (name,category,sku,unit,cost,price,qty,reorder) and
  // adds every valid row as a new item in one go.
    ...
    reader.onload = () => {
      try {
        ...
        const parseLine = (line) => {
          // Handles simple quoted CSV fields (e.g. "Samsung, 55\"")
          const out = [];
          let cur = "";
          let inQuotes = false;
        ...
        if (lines.length < 2) {
          log("CSV import: no data rows found.");
          return;
        }
        ...
        if (newItems.length === 0) {
          log("CSV import: couldn't find any valid rows (check the column headers).");
          return;
        }
        setItems((arr) => [...newItems, ...arr]);
        log(`Imported ${newItems.length} item(s) from CSV.`);
      } catch (err) {
        log("CSV import failed — please check the file format.");
      }
    };
  ...
  function proposeDeal(itemId, percent, weekLabel, endDate) {
    const itemName = items.find((i) => i.id === itemId)?.name;
    setItems((arr) =>
      arr.map((i) => (i.id === itemId ? { ...i, dealStatus: "pending", dealPercent: Number(percent), dealWeek: weekLabel, dealEndDate: endDate } : i))
    );
  ...
  React.useEffect(() => {
    if (!ready) return;
    const isNotExpired = (i) => !i.dealEndDate || new Date(i.dealEndDate + "T23:59:59").getTime() >= Date.now();
    const approvedDeals = items
      .filter((i) => i.dealStatus === "approved" && i.qty > 0 && isNotExpired(i))
      .map((i) => ({
        id: i.id, name: i.name, category: i.category, price: i.price,
        dealPercent: i.dealPercent, dealWeek: i.dealWeek, unit: i.unit, image: i.image || "", qty: i.qty, dealEndDate: i.dealEndDate || "", createdAt: i.createdAt || 0,
      }));
  ...
  return (
    <LangContext.Provider value={{ t, lang, setLang }}>
    <div className={`min-h-screen flex bg-[#F5F6F8] font-[Inter] text-slate-800 ${darkMode ? "app-dark" : ""}`}>
      <style>{`
      ...
      >
        ...
        <div className="px-5 pt-4 flex items-center gap-2">
          ...
          <button
            ...
            className="h-[26px] w-[26px] rounded-lg border border-white/20 flex items-center justify-center text-slate-300 hover:bg-white/10 transition-colors"
          ...
          {visibleNav.map((n) => {
            ...
            return (
              ...
              >
                ...
                {navBadges[n.key] > 0 && (
                  <span className="text-[10px] font-bold bg-red-500 text-white rounded-full h-4 min-w-[16px] px-1 flex items-center justify-center">
                    {navBadges[n.key]}
                  </span>
                )}
              </button>
...
function ItemsView({ items, warehouses, onAdd, onEdit, onDelete, warehouseName, role, onProposeDeal, onApproveDeal, onRejectDeal, onEndDeal, onImportCsv }) {
  const { t } = useLang();
  const [q, setQ] = useState("");
    ...
    <div className="space-y-6">
      ...
      <Card className="p-5">
        <div className="flex items-center justify-between mb-4 gap-3">
          ...
          <div className="flex items-center gap-2">
            <label className="px-3.5 py-2 rounded-lg text-sm font-semibold border border-slate-200 text-slate-700 hover:bg-slate-50 cursor-pointer inline-flex items-center gap-1.5">
              <FileUp size={15} /> Import CSV
              <input type="file" accept=".csv,text/csv" className="hidden" onChange={(e) => { if (e.target.files[0]) { onImportCsv(e.target.files[0]); e.target.value = ""; } }} />
            </label>
            <Btn onClick={onAdd}>
        ...
        <p className="text-[11px] text-slate-400 mb-3 -mt-2">
          CSV columns: name, category, sku, unit, cost, price, qty, reorder (first row = headers)
        </p>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <div className="font-medium text-slate-700 flex items-center gap-1.5">
                        {i.name}
                        {i.createdAt && Date.now() - i.createdAt < 14 * 24 * 60 * 60 * 1000 && (
                          <span className="text-[9px] font-extrabold px-1.5 py-0.5 rounded-full bg-emerald-100 text-emerald-700">NEW</span>
                        )}
                      </div>
      ...
      {dealModalItem && (
        <DealModal
          ...
          onSubmit={(percent, week, endDate) => {
            onProposeDeal(dealModalItem.id, percent, week, endDate);
  ...
  return (
    <Modal title={`Propose a deal — ${item.name}`} onClose={onClose}>
      <form
        onSubmit={(e) => {
          e.preventDefault();
          onSubmit(Number(percent), week, endDate);
        }}
        className="flex flex-col gap-4"
      >
      ...
      try {
        ...
        const scanLoop = async () => {
          ...
          try {
            ...
            // detection hiccup — just try again next frame
          }
          rafId = requestAnimationFrame(scanLoop);
        };
        scanLoop();
      } catch (e) {
        setError("Couldn't access the camera — check that you've allowed camera permission.");
      }
    })();
    return () => {
      stopped = true;
    ...
    <Modal title="Scan a barcode" onClose={onClose}>
      {!supported ? (
        <p className="text-sm text-slate-600">
          Barcode scanning isn't supported in this browser yet. It works best in Chrome on Android — try typing the SKU instead.
        </p>
      ) : error ? (
...
function ReportsView({ items, sales }) {
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="p-5">
        <div className="flex items-center justify-between mb-4">
          <h3 className="font-[Manrope] font-bold text-sm text-slate-700">Storefront analytics</h3>
          {analytics && <span className="text-xs text-slate-400">{analytics.visits} total visits</span>}
        </div>
        {!analytics ? (
          <p className="text-sm text-slate-400 py-4 text-center">Loading…</p>
        ) : analytics.items.length === 0 ? (
          <p className="text-sm text-slate-400 py-4 text-center">No "Buy on Telegram" clicks recorded yet.</p>
        ) : (
          <div className="space-y-2">
            <p className="text-xs text-slate-500 mb-2">Most-clicked deals on the storefront</p>
            {analytics.items.map((it, idx) => (
              <div key={it.id} className="flex items-center justify-between text-sm border-b border-slate-50 pb-1.5 last:border-0">
                <span className="text-slate-600">{idx + 1}. {it.name || "(deleted item)"}</span>
                <span className="font-mono font-semibold text-slate-700">{it.count} clicks</span>
              </div>
            ))}
          </div>
        )}
