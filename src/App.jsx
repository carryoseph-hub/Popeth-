    ...
    <div className="min-h-screen flex items-center justify-center bg-[#F5F6F8] font-[Inter] p-4">
      ...
      <div className="bg-white rounded-2xl shadow-lg p-8 w-full max-w-sm">
        ...
        >
          ...
          <Btn type="submit" className="justify-center mt-2">{loading ? t("signing_in") : t("sign_in")}</Btn>
        </form>
      </div>
    </div>
  );
...
function Storefront({ onStaffLogin }) {
  ...
  const visibleDeals = useMemo(
    () => (deals || []).filter((d) => activeCat === "All" || d.category === activeCat),
    [deals, activeCat]
  );
    ...
    <div className="min-h-screen bg-[#F5F6F8] font-[Inter] text-slate-800">
      ...
      >
        ...
        <div className="max-w-5xl mx-auto relative z-10 mt-10">
          ...
          <h1 className="font-[Manrope] font-extrabold text-white text-2xl md:text-4xl leading-tight">{t("this_weeks_deals")}</h1>
          <p className="text-slate-300 mt-2 max-w-lg">{t("storefront_tagline")}</p>
        </div>
                ...
                return (
                  <Card key={d.id} className="overflow-hidden group cursor-default">
                    <div
                      ...
                      style={{ background: `linear-gradient(135deg, ${catColor(d.category)}22, ${catColor(d.category)}0A)` }}
                    >
                      {d.image ? (
                        <img
                          src={d.image}
                        ...
                        <Package size={40} style={{ color: catColor(d.category) }} className="opacity-40" />
                      )}
                      <span className="absolute top-2.5 left-2.5 flex items-center gap-1 text-[11px] font-extrabold px-2 py-1 rounded-full bg-red-500 text-white shadow-sm">
                        <Zap size={10} fill="white" /> -{d.dealPercent}%
                      </span>
                    </div>
                    <div className="p-3.5">
                      <div className="text-[10px] font-semibold uppercase tracking-wide" style={{ color: catColor(d.category) }}>{d.category}</div>
                      ...
                      <div className="text-[11px] text-emerald-600 font-semibold mt-0.5">Save {money(savings)}</div>
                    </div>
                  </Card>
                );
              })}
  ...
  React.useEffect(() => {
    ...
    const approvedDeals = items
      ...
      .map((i) => ({
        ...
        dealPercent: i.dealPercent, dealWeek: i.dealWeek, unit: i.unit, image: i.image || "",
      }));
    ...
    <div className="space-y-6">
      ...
      <Card className="p-5">
        ...
        >
          ...
          <Btn type="submit" variant="outline"><Plus size={14} /> {t("assign")}</Btn>
        </form>
      </Card>
